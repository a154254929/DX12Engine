    ...
    ANALYSIS_HRESULT(graphicsCommandList->Reset(commandAllocator.Get(), NULL));
    {
        ...
        */
        ...
        if (GBoxMesh* boxMesh = world->CreateActorObject<GBoxMesh>())
        {
            ...
            {
                material->SetBaseColor(fvector_4d(1.f, 1.f, 1.f, 1.f));
                material->SetBaseColorIndexKey(FEnginePathHelper::GetEngineRelativeContentPath() + "/Texture/Texture2.dds");
                material->SetMaterialType(EMaterialType::BaseColor);
                material->SetFresnelF0(fvector_3d(1.f));
            }
        }
        ...
        if (GSphereMesh* sphereMeshLambertain = world->CreateActorObject<GSphereMesh>())
        {
            ...
            sphereMeshLambertain->SetPosition(XMFLOAT3(-4, 1, 4));
            {
                if (CMaterial* material = (*sphereMeshLambertain->GetMaterials())[0])
                {
                    ...
                    material->SetBaseColorIndexKey(FEnginePathHelper::GetEngineRelativeContentPath() + "/Texture/Texture2.dds");
                    material->SetNormalMapIndexKey(FEnginePathHelper::GetEngineRelativeContentPath() + "/Texture/Texture2_NRM.dds");
        ...
        if (GSphereMesh* sphereMeshBanded = world->CreateActorObject<GSphereMesh>())
        {
            ...
            sphereMeshBanded->SetPosition(XMFLOAT3(4, 3.5, 0));
            {
                if (CMaterial* material = (*sphereMeshBanded->GetMaterials())[0])
                {
                    material->SetBaseColor(fvector_4d(.8f, .5f, .5f, 1.f));
                    material->SetRoughness(.95f);
                    material->SetMaterialType(EMaterialType::Banded);
                }
            }
        }
