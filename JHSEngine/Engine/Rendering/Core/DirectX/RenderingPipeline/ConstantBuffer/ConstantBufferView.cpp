...
void FConstantBufferView::BuildConstantBuffer(CD3DX12_CPU_DESCRIPTOR_HANDLE inHandle, UINT inCinstantBufferNum)
{
    ...
    for (int i = 0; i < inCinstantBufferNum; ++i)
    {
        ...
        objCBVDesc.BufferLocation = objAddr + constants->GetConstantBufferByteSize() * i;
        objCBVDesc.SizeInBytes = constants->GetConstantBufferByteSize();

        inHandle.Offset(i, descriptorOffset);
