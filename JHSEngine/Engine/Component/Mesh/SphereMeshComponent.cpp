...
void CSphereMeshComponent::CreateMesh(FMeshRenderingData& meshRenderingData, float inRadius, uint32_t inAxialSubdivision, uint32_t inHeightSubdivision)
{
    ...
    for (uint32_t j = 0; j < inAxialSubdivision; ++j)
    {
        meshRenderingData.vertexData.push_back(FVertex(
            ...
            XMFLOAT3(0.f, 1, 0.f),
            XMFLOAT2((j + .5f) * 1.0f / inAxialSubdivision , 0.f)
        ));
    }
    for (uint32_t i = 1; i < inHeightSubdivision; ++i)
    {
        ...
        uint32_t roundStart = (i - 1) * (inAxialSubdivision + 1) + inAxialSubdivision;
        for (uint32_t j = 0; j <= inAxialSubdivision; ++j)
        {
    ...
    for (uint32_t j = 0; j < inAxialSubdivision; ++j)
    {
        meshRenderingData.vertexData.push_back(FVertex(
            ...
            XMFLOAT2((j + .5f) * 1.0f / inAxialSubdivision, 1.f)
        ));
    }
    uint32_t finalIndex = (inHeightSubdivision - 1) * (inAxialSubdivision + 1) + inAxialSubdivision;
